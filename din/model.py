...
class Model(object):
  ...
  def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
    ...
    h_emb = tf.concat([    # 用户行为序列(User Behavior)物品的item_emb和cate_emb拼接，shape: [B, T, H]
        tf.nn.embedding_lookup(item_emb_w, self.hist_i),
        tf.nn.embedding_lookup(cate_emb_w, hc),
        ], axis=2)
    print("h_emb", h_emb.get_shape().as_list())
    ...
    self.logits = i_b + d_layer_3_i #最后的结果
...
def attention(queries, keys, keys_length):
    ...
    d_layer_3_all = tf.layers.dense(d_layer_2_all, 1, name='f3_att', reuse=tf.AUTO_REUSE)
